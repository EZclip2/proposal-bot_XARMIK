...
function cardText(rec) {
  ...
  return `${gem}🟢 <b>Обработано · №${rec.id}</b>\n${head}\n\n${body}` +
...
async function createSuggestion(msg, kind) {
  ...
  const rec = {
    ...
    answerText: null, modMessageId: null, userMsgId: null,
...
async function rejectSuggestion(sid) {
  ...
  if (rec.userMsgId) {
    try {
      await bot.editMessageText(`✅ Ваше предложение №${rec.id} рассмотрено администрацией.`,
...
app.post('/api/answer', async (req, res) => {
  ...
  // деловые остаются в своей теме (важный тред не разбиваем), обычные → в «Обработанные»

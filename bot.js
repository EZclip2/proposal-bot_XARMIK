...
if (typeof db.counter !== 'number' || Number.isNaN(db.counter)) db.counter = 100; // защита номеров
...
async function updateQueue() {
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    try {
      await bot.editMessageText(queueText(i),
...
async function createSuggestion(msg, kind) {
  ...
  // чистим прошлые служебные сообщения бота у подписчика
  ...
  try {
    ...
    if (isBiz) {
      const conf = await bot.sendMessage(msg.chat.id,
        '💎 Ваше деловое предложение принято и передано администрации напрямую. С вами свяжутся.',
      ...
      const conf = await bot.sendMessage(msg.chat.id, queueText(ahead),
        { parse_mode: 'HTML', reply_markup: cancelKb(rec.id) });
      rec.userMsgId = conf.message_id;
  ...
  if (!wasAnswered) {
    ...
    try {
      const n = await bot.sendMessage(rec.userId,
        '🚫 <b>Ваше предложение отклонено администрацией.</b>\n⏳ Отправить новое можно будет через 15 минут.',
...
async function cancelSuggestion(sid, q) {
  ...
  try {
    await bot.editMessageText('🚫 Обращение отменено.',
      { chat_id: rec.userId, message_id: rec.userMsgId, reply_markup: HOME_KB });
...
bot.on('callback_query', async (q) => {
  ...
  try {
    if (data.startsWith('kind:')) {
      ...
      try {
        await bot.editMessageText(txt, { chat_id: q.message.chat.id, message_id: q.message.message_id, reply_markup: HOME_KB });
        queueCleanup(q.from.id, q.message.message_id);
      } catch {
        const p = await bot.sendMessage(q.message.chat.id, txt, { reply_markup: HOME_KB });
...
app.post('/api/answer', async (req, res) => {
  ...
  if (isFirst) {
    ...
    // убираем живой статус очереди у подписчика — ответ уже пришёл
    if (rec.userMsgId) { try { await bot.deleteMessage(rec.userId, rec.userMsgId); } catch {} }
  } else {
    await refreshCard(rec);
  }

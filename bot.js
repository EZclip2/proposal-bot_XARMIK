...
// Модератор мог открыть мини-приложение и не ответить — возвращаем карточку в «Новые».
...
async function rejectSuggestion(sid) {
  ...
  // мут и уведомление об отказе — только если предложению ещё НЕ ответили
  const wasAnswered = rec.state === 'answered' || (rec.answers && rec.answers.length > 0);
  if (!wasAnswered) {
    muteUser(rec.userId); // мут на 15 минут
    if (rec.userMsgId) {
      try {

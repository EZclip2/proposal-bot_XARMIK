...
let db = { counter: 100, items: {}, mutes: {} };
...
// Временный выбор категории пользователем (обычное / деловое). Не персистим —
// по умолчанию всё считается обычным предложением.
...
const START_KB = { inline_keyboard: [
  [{ text: '✍️ Обычное предложение', callback_data: 'kind:normal' }],
  [{ text: '💎 Деловое предложение', callback_data: 'kind:biz' }],
] };
...
function cardText(rec) {
  ...
  if (rec.state === 'new') {
    return `${isBiz ? '💎' : '🟡'} <b>Новое ${noun} №${rec.id}</b>\n${head}\n\n${body}`;
  }
  if (rec.state === 'processing') {
    return `${gem}🟠 <b>Обрабатывается · №${rec.id}</b>\n${head}\n🖊 В работе: ${esc(rec.moderatorName)}\n\n${body}`;
  }
  // answered — основной ответ + все дополнения
  let out = `${gem}🟢 <b>Обработано · №${rec.id}</b>\n${head}\n\n${body}`;
...
// Перенос обработанной карточки в тему «Обработанные» (только для обычных предложений).
// Telegram не умеет двигать сообщения между темами — пересоздаём и удаляем старую.
async function relocateAnswered(rec) {
  ...
  try {
    ...
    await refreshCard(rec); // запасной вариант — отредактировать на месте
  }
}
...
function queueText(rec, ahead) {
  const base = `✅ Предложение №${rec.id} принято!`;
  if (ahead <= 0) return `${base}\n\n⏳ Ваш вопрос первый в очереди — скоро ответим 💜`;
  return `${base}\n\n⏳ Перед вами в очереди: <b>${ahead}</b>\n<i>Счётчик обновляется автоматически.</i>`;
}
  ...
  for (let i = 0; i < pending.length; i++) {
    ...
    try {
      await bot.editMessageText(queueText(rec, i),
        { chat_id: rec.userId, message_id: rec.userMsgId, parse_mode: 'HTML' });
      rec.lastShownPos = i;
      persist();
    } catch (e) {
      rec.lastShownPos = i; // сообщение удалено / бот заблокирован — не долбим API
    }
  }
}
...
async function createSuggestion(msg, kind) {
  ...
  const rec = {
    id: store.nextId(), kind: isBiz ? 'biz' : 'normal',
    userId: from.id, userMention: mention,
    isMedia: !isText, bodyText: msg.text || msg.caption || '',
    state: 'new', moderatorId: null, moderatorName: null,
  ...
  try {
    ...
    // деловые → в свою тему, обычные → в тему «Новые»
    const topic = isBiz ? TOPIC_BIZ : TOPIC_NEW;
    const thread = topic ? { message_thread_id: Number(topic) } : {};
    let sent;
    if (isText) {
      sent = await bot.sendMessage(MOD_CHAT_ID, cardText(rec),
        { parse_mode: 'HTML', reply_markup: kb, ...thread });
    } else {
    ...
    if (isBiz) {
      // деловые не встают в общую очередь — приоритетная обработка
      const conf = await bot.sendMessage(msg.chat.id,
        `💎 Ваше деловое предложение №${rec.id} принято и передано администрации напрямую. С вами свяжутся.`,
        { parse_mode: 'HTML' });
      rec.userMsgId = conf.message_id;
    } else {
      const ahead = store.all().filter(r =>
        r.kind !== 'biz' && (r.state === 'new' || r.state === 'processing') && r.id < rec.id).length;
      const conf = await bot.sendMessage(msg.chat.id, queueText(rec, ahead), { parse_mode: 'HTML' });
      rec.userMsgId = conf.message_id;
...
async function rejectSuggestion(sid) {
  ...
  // мут на 15 минут
  muteUser(rec.userId);
  // убираем счётчик очереди у подписчика
  if (rec.userMsgId) {
    try {
      await bot.editMessageText(`🚫 Ваше предложение №${rec.id} отклонено.`,
        { chat_id: rec.userId, message_id: rec.userMsgId });
    } catch {}
  }
  // отдельное уведомление (с пушем)
  try {
    await bot.sendMessage(rec.userId,
      `🚫 <b>Ваше предложение №${rec.id} отклонено администрацией.</b>\n⏳ Отправить новое предложение можно будет через 15 минут.`,
      { parse_mode: 'HTML' });
  } catch {}
...
bot.onText(/^\/start/, (msg) => {
  bot.sendMessage(msg.chat.id,
    '👋 Привет! Напиши своё предложение, вопрос или отзыв — по умолчанию это обычное предложение.\n\n' +
    '💎 Если у тебя <b>деловое предложение</b> (реклама, сотрудничество и т.п.) — нажми кнопку ниже, оно уйдёт администрации напрямую.',
    { parse_mode: 'HTML', reply_markup: START_KB });
});

bot.on('message', async (msg) => {
  // Служебная команда для настройки тем форума
  ...
  // мут после отклонения
  const muteLeft = mutedFor(msg.from.id);
  if (muteLeft > 0) {
    const mins = Math.ceil(muteLeft / 60000);
    await bot.sendMessage(msg.chat.id,
      `⛔ Ваше предыдущее предложение было отклонено. Отправить новое можно через ~${mins} мин.`);
    return;
  }

  const kind = userMode.get(msg.from.id) === 'biz' ? 'biz' : 'normal';
  userMode.delete(msg.from.id); // сбрасываем — следующее сообщение снова обычное
...
bot.on('callback_query', async (q) => {
  ...
  try {
    if (data.startsWith('kind:')) {
      ...
      userMode.set(q.from.id, kind === 'biz' ? 'biz' : 'normal');
      const txt = kind === 'biz'
        ? '💎 Опишите ваше деловое предложение одним сообщением (можно с файлами) — оно уйдёт администрации напрямую.'
        : '✍️ Напишите ваше предложение, вопрос или отзыв.';
...
app.post('/api/answer', async (req, res) => {
  ...
  const isFirst = rec.state !== 'answered'; // первый ответ или дополнение
  ...
  if (isFirst) {
    // деловые остаются в своей теме, обычные → в «Обработанные»
    if (rec.kind !== 'biz' && TOPIC_ANSWERED) await relocateAnswered(rec);
    else await refreshCard(rec);
    if (rec.userMsgId) {
      try {
        await bot.editMessageText(`✅ На ваше предложение №${rec.id} ответили 👇`,
          { chat_id: rec.userId, message_id: rec.userMsgId });
      } catch {}
    }
  } else {
    // дополнение — карточка уже в нужной теме, просто обновляем на месте
    await refreshCard(rec);
  }
...
app.listen(PORT, '0.0.0.0', async () => {
  ...
  setInterval(() => { updateQueue().catch(e => console.error('updateQueue:', e.message)); }, 60000);
});
